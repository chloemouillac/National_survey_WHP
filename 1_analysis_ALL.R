...
all_data$ESPECE_mode[all_data$ESPECE_mode=="Oui"] <- "Mode"
all_data$ESPECE_mode[all_data$ESPECE_mode=="Non"] <- "Pas de mode"

all_data$ESPECE_mode_risque[all_data$ESPECE_mode_risque=="Oui"] <- "Mode et risque"
all_data$ESPECE_mode_risque[all_data$ESPECE_mode_risque=="Non"] <- "Mode sans risque"

all_data$ESPECE_espece_reglem[grepl("Oui", all_data$ESPECE_espece_reglem)] <- "Oui"
all_data$ESPECE_espece_reglem[grepl("Non", all_data$ESPECE_espece_reglem)] <- "Non"

all_data$PROFIL_statut_cueilleur[grepl("loisir", all_data$PROFIL_statut_cueilleur)] <- "Amateur"
all_data$PROFIL_statut_cueilleur[grepl("revenus", all_data$PROFIL_statut_cueilleur)] <- "Professionnel"
...
all_data <- all_data %>%
  ...
  mutate(PROFIL_statut_cueilleur = if_else(PROFIL_cueilleur == "Oui", 
                                           PROFIL_statut_cueilleur, "Non cueilleur")) %>%
...
all_data_ACM_profil <- all_data %>%
  select(id, ESPECE_species,
         ...
         # ESPECE_reglem_adaptee, ESPECE_risque,
...
##### PERSONAL NOTES ####
# Faire un chi2 pour l'analyse des espèces types par cluster ? beaucoup d'individus seuls...
# Dégrader au genre pour certaines espèces (genepi...) pour les grouper et leur donner plus de poids ?



##### Multivariate : profiles ####
all_data$PROFIL_nb_esp_cueill[is.na(all_data$PROFIL_nb_esp_cueill)] <- 0

all_data_profiles_ACM <- all_data %>%
...
res.mca <- MCA(all_data_profiles_ACM %>% select(-id),
               graph = FALSE)
...
# Results :
# 
# Cluster 1 : Perception de cueillette durable 
# 
# Caractéristiques des espèces
#   Durabilité et ressource :
#   98 % des espèces sont considérées comme durables, ce qui est nettement supérieur à la moyenne globale de 65 %.
#   74 % des espèces ont un état de ressource stable, comparé à 47 % globalement.
#   57 % sont abondantes à large échelle (33 % globalement).
# 
#   Cueillette et prélèvement :
#   82 % des espèces ne subissent pas d'effet de mode, alors que la moyenne globale est de 61 %.
#   L'étendue de la cueillette est généralisée pour 80 % des espèces, contre 63 % globalement.
#   La variation de prélèvement est stable pour 43 % des espèces, ce qui est supérieur à la moyenne globale de 28 %.
#   L'intensité de prélèvement est principalement faible (36 %) ou moyenne (31 %), comparé à une moyenne globale de 25 % et 23 % respectivement.
# 
#   Usages :
#   87 % des espèces ont un usage alimentaire, plus élevé que la moyenne globale de 75 %.
#   98 % n'ont pas d'usage ornemental, contre 90 % globalement.
#   50 % des espèces ont un usage médical, légèrement supérieur à la moyenne globale de 45 %.
# 
#   Parties cueillies :
#   Les feuilles sont cueillies pour 53 % des espèces, contre une moyenne globale de 41 %.
#  Les parties souterraines ne sont pas cueillies pour 96 % des espèces, ce qui est très proche de la moyenne globale de 90 %.
# 
# Catégories sous-représentées
#  Les espèces non durables, celles dont la ressource diminue, celles subissant des effets de mode et les espèces rares.
# 
# Profil des répondants
#   Catégorie socioprofessionnelle :
#   18 % des répondants sont des retraités, alors que leur proportion globale est de 14 %.

#   Type d'organisation :
#   52 % des répondants ne sont rattachés à aucune organisation, ce qui est similaire à la moyenne globale de 48 %.
#   
#   Catégories sous-représentées
#   Les  moins de 26 ans sont sous-représentées, ne constituant que 4 % du cluster, contre près de 6 % globalement.
#   Les répondants rattachés à la police de l'environnement sont également peu représentés, avec seulement 2 % du cluster, par rapport à une moyenne globale de 7 %.
...
#   Les espèces de ce cluster sont souvent considérées comme non durables (50 %), ce qui est bien supérieur à la moyenne globale de 35 %.
#   Leur présence est principalement abondante localement (70 %, contre 50 % au global) plutôt que largement répandue.
#   L'état des ressources est sovuent inconnu (25 %, contre 16 % au global).
# 
# 
#   Prélèvement et usages :
#   Le prélèvement subit des effets de mode (52 %, contre 39 % au global) et souvent variable en intensité spatialement (51 %, contre 24 % au global) et temporellement (46 %, contre 21 % au global).
#   Le cluster est fortement associé à un effet de mode sur l'espèce (52 % contre 39 % globalement).
#   L'usage ornemental est surreprésenté (28 %, contre 10% au global). 
#   L'usage alimentaire est sous-représenté (62 % contre 75 % globalement).
# 
#  Les parties cueillies sont rarement les feuilles (79 % ne le sont pas, 59 % globalement), au profit de la plante entière (24 %, 16 % globalement) ou de parties aériennes spécifiques comme la sève ou les bourgeons.
# 
#
# Profil des répondants
#   Rattachement professionnel :
#   Les répondants sont principalement rattachés à la police de l'environnement (30 %, contre 7 % globalement) ou sont des gestionnaires d'espaces (18 %, contre 7 % globalement).
#                                                                          
#   La catégorie socioprofessionnelle est dominée par les employés (33 %) et les cadres (42 %).
#                                                                          
#   Caractéristiques démographiques et expérience :
#   Ce sont majoritairement des hommes (60 %) et des jeunes (18 % ont moins de 26 ans, contre 6 % au global).
#   La majorité ne cueille qu'entre 1 et 5 espèces (55 %, 33 % globalement).
...
# 
# Durabilité et ressources :
# Les espèces de ce cluster sont majoritairement non durables (90 %), ce qui est très supérieur à la moyenne globale de 35 %.
# L'état des ressources est en déclin (72 %), comparé à 35 % globalement.
# Ces espèces sont souvent rares (38 %), contre 13 % au global, ou même extrêmement rares (5 %), contre 2 % au global.
# 
# Prélèvement et usages :
# Le prélèvement subit des effets de mode (68 %), contre 39 % globalement, avec une intensité forte (31 %), contre 13 % au global, et est en augmentation (42 %), contre 23 % au global.
# L'étendue de la cueillette est localisée (36 %), contre 20 % au global, ou très localisée (9 %), contre 5 % au global.
# L'usage alimentaire est sous-représenté (39 %), contre 75 % globalement.
# La cueillette se concentre sur les fleurs (54 %), contre 40 % globalement, et les parties souterraines (16 %), contre 10 % globalement.
# 
# Profil des répondants
# Rattachement professionnel :
# La majorité des répondants n'a aucune organisation de rattachement (60 %), contre 48 % globalement.
# On trouve une forte représentation des agriculteurs (21 %), contre 12 % globalement, et des artisans (17 %), contre 10 % globalement.
# 
# Caractéristiques démographiques et expérience :
# Une grande partie des répondants ont plus de 50 ans (55 %), contre 47 % globalement.
# Les répondants sont des femmes (64 %), contre 57 % globalement.
# Plus de la moitié d'entre eux qui cueillent entre 5 et 30 espèces (57 %), contre 48 % globalement, et près d'un quart qui en cueillent entre 30 et 100 (22 %, contre 6 % globalement).
# Les jeunes de moins de 26 ans sont totalement absents de ce cluster.
