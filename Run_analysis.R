...
all_rarity <- read.csv("raw_data/OpenObs+GBIF_RARITY_20km.csv") %>%
  ...
  mutate(
    dpt_simple = str_replace_all(dpt_name, "[-']", " "),
    dpt_simple = iconv(dpt_simple, to = "ASCII//TRANSLIT")
  )

all_rarity <- all_rarity %>% left_join(all_rarity %>% group_by(CD_REF) %>%
...
plot_harvest_areas <- ggplot() +
  ...
  geom_label(
    data = centroids, aes(label = harvesting_area_letters, geometry = geometry),
...
process_all_species_data <- function(data) {
  ...
  species_cols <- data %>%
    dplyr::select(
      ...
      starts_with("GsuppQsupp_")
    ) # GsuppQsupp__Connaissez_vous_une_espece_a_la_cueillette_unsustainable
...
age <- ggplot(
  all_data %>%
    ...
    unique() %>%
    mutate(
      PROFILE_age = ifelse(is.na(PROFILE_age),
        "Prefer not to answer",
        as.character(PROFILE_age)
      ),
      PROFILE_age = factor(PROFILE_age,
        levels =
...
ord <- c(
  setdiff(ord, c("Other", "Prefer not to answer")),
...
sp_mentions <- all_data %>%
  group_by(CD_REF, SPECIES_name) %>%
...
summary_unsustainable_species <- df_all_species_data %>%
  ...
  group_by(name) %>%
  summarise(
    Unsustainable = sum(Unsustainable, na.rm = T),
...
# Prepare datasets
data_rarity_plot <- all_data %>%
  select(CD_REF, sp_area_FR) %>%
  unique() %>%
...
France <- all_rarity %>%
  group_by(dpt_name) %>%
  summarise(dpt_area = max(dpt_area)) %>%
  summarise(total_area = sum(dpt_area)) %>%
  pull(total_area)
...
ggplot(data_rarity_plot, aes(x = 100 * sp_area_FR / France, fill = in_survey)) +
  ...
  geom_vline(
    xintercept = 100 * quantiles_all / France,
    linetype = "dashed", color = "blue", size = 1
  ) +
  geom_vline(
    xintercept = 100 * quantiles_survey / France,
...
plot_sustainability_ratio(
  all_data %>% mutate(relative_sp_area_FR = 100 * sp_area_FR / France) %>%
    filter(relative_sp_area_FR < 20 &
      native == "native"),
  n = NULL,
  min_ratio = 0, min_citations = 2, max_citations = 7
) # there are no species under 50% unsustainable ratio !
...
all_data_ACM_participant_profile <- all_data %>%
  dplyr::select(
    id, PROFILE_harvester_status, PROFILE_affiliated_organisation_type,
...
var_data <- var_data %>%
  mutate(
    contrib.Dim.1 = var_contrib$`Dim 1`,
...
plot_MCA_with_vars <- plot_MCA_particip +
  geom_segment(
    data = var_data, aes(x = 0, y = 0, xend = `Dim 1`, yend = `Dim 2`),
    arrow = arrow(length = unit(0.2, "cm")), color = "black", size = 0.2
  ) +
  geom_text_repel(
    data = var_data, aes(x = `Dim 1`, y = `Dim 2`, label = rownames(var_data)),
    color = "black", size = 4
  )
...
# Contingency table
table_gender_cluster <- table(
  cluster_gender$clust,
